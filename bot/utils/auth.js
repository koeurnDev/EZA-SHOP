...
function validateInitData(telegramInitData, botToken) {
  ...
  try {
    ...
    // 6. Compare hashes securely (wrapped in try/catch to catch non-hex malformed hash Buffer.from errors)
    ...
    console.error('🔴 Telegram InitData Validation Exception:', err.message);
    return false;
  }
}
