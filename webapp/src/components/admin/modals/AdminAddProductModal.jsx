    ...
    <div className="admin-dashboard-overhaul" style={{ position: 'fixed', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, background: 'var(--bg-soft)' }}>
      <div className="glass-card-luxury" style={{ width: '92%', maxWidth: 440, maxHeight: '85vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 15, flexShrink: 0 }}>
          <button className="nav-pill-btn btn-destructive" style={{ flex: 1, minHeight: 48, padding: '0 5px' }} disabled={isSaving} onClick={() => setIsAddingProduct(false)}>បោះបង់</button>
          <button className="nav-pill-btn" style={{ flex: 1, minHeight: 48, padding: '0 5px', background: 'var(--bg-surface)', color: 'var(--text-main)', border: '1px solid #e2e8f0' }} onClick={() => handlePreview(newProductData)}>👁️ មើលសិន</button>
