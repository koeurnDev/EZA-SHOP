...
const AdminAddProductModal = React.memo(({
  ...
  const categoryInList = PREDEFINED_CATEGORIES.some(c => c.id === newProductData.category);
  const categorySelectValue = categoryInList ? newProductData.category : 'OTHER';
  const showSkincareHint = isSkincareProduct({ category: newProductData.category, name: newProductData.name });

  return (
    <div className="admin-coupon-form" style={{ marginTop: 10, display: 'flex', flexDirection: 'column' }}>
      <h3 className="admin-coupons-title" style={{ marginBottom: 16 }}>{t('admin_add_product')}</h3>
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <textarea className="input-glass-admin" rows="3" value={newProductData.description} onChange={e => setNewProductData({ ...newProductData, description: e.target.value })} placeholder={t('admin_product_desc')} />
        </div>
        ...
        <div style={{ marginBottom: 14, padding: '12px 14px', background: 'var(--bg-soft)', borderRadius: 12, border: '1px solid var(--border-soft)' }}>
          ...
          <div style={{ marginBottom: 10 }}>
            ...
            <textarea
              className="input-glass-admin"
              rows="3"
              value={newProductData.howToUse || ''}
              onChange={e => setNewProductData({ ...newProductData, howToUse: e.target.value })}
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
          <button className="nav-pill-btn btn-destructive"
            style={{ flex: 1 }} disabled={isSaving} onClick={() => setIsAddingProduct(false)}>{t('cancel')}</button>
          <button className="nav-pill-btn"
            style={{ flex: 1 }} disabled={isSaving} onClick={() => handlePreview({ ...newProductData, id: 'PREVIEW' })}>{t('admin_preview')}</button>
          <button className="ticket-btn-primary"
            style={{ flex: 1.2 }} disabled={isUploading || isSaving} onClick={submitAddProduct}>
